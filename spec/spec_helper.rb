...
VCR.configure do |c|
  ...
  c.filter_sensitive_data('<OKAPI>') { ENV['OKAPI_URL'] }
  ...
  # c.filter_sensitive_data('<UUID>') do |interaction|
  #  # interaction.response.body.gsub(/\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b/, '<UUID>')
  # end
  ...
  #   interaction.request.uri.gsub!('<OKAPI URL>', ENV['OKAPI_URL'])
  # end
end